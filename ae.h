...
typedef void ae_file_proc(struct ae_event_loop *event_loop, int fd, void *client_data, int mask);
typedef int ae_time_proc(struct ae_event_loop *event_loop, long long id, void *client_data);
typedef void ae_event_finalizer_proc(struct ae_event_loop *event_loop, void *client_data);
typedef void ae_before_sleep_proc(struct ae_event_loop *event_loop);
...
typedef struct ae_event_loop {
    ...
    ae_file_event   events[AE_SETSIZE];
    ae_fired_event  fired[AE_SETSIZE];
...
void ad_stop(ae_event_loop *event_loop);

int ae_create_file_event(ae_event_loop *event_loop, int fd, int mask,
...
int ae_process_event(ae_event_loop *event_loop, int flags);
